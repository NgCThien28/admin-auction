...
const selectedDepositForCancel = ref(null);
...
// Cancel functions
const openCancelModal = (deposit) => {
  selectedDepositForCancel.value = deposit;
  cancelTitle.value = "";
  cancelContent.value = "";
...
const closeCancelModal = () => {
  ...
  selectedDepositForCancel.value = null;
  cancelTitle.value = "";
  cancelContent.value = "";
};

const submitCancel = async () => {
  if (!selectedDepositForCancel.value || !cancelTitle.value.trim() || !cancelContent.value.trim()) {
  ...
  try {
    await axios.put(
      `http://localhost:8082/api/deposit-payments/cancel-deposit/${selectedDepositForCancel.value.matc}`,
      {
  ...
  <div class="bg-white border border-slate-200 rounded-sm">
    ...
    >
      <div class="bg-white rounded-lg shadow-lg max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
          <h2 class="text-lg font-semibold text-slate-800">Chi tiết phiếu tiền cọc</h2>
          <button
            @click="closeModal"
        ...
        <div v-if="selectedDeposit" class="px-6 py-4 space-y-3">
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Mã phiếu:</span>
            <span class="text-sm text-slate-800">{{ selectedDeposit.matc }}</span>
          </div>
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Mã phiên đấu giá:</span>
            <span class="text-sm text-slate-800">{{ selectedDeposit.phienDauGia?.maphiendg }}</span>
          </div>
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Mã khách hàng:</span>
            <span class="text-sm text-slate-800">{{ selectedDeposit.taiKhoanKhachThanhToan?.matk }}</span>
          </div>
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Ngày thanh toán:</span>
            <span class="text-sm text-slate-800">{{ formatDateTime(selectedDeposit.thoigianthanhtoan) }}</span>
          </div>
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Hạn thanh toán:</span>
            <span class="text-sm text-slate-800">{{ formatDateTime(selectedDeposit.hanthanhtoan) }}</span>
          </div>
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Số tiền:</span>
            <span class="text-sm text-slate-800">{{ formatMoney(selectedDeposit.sotien) }}</span>
          </div>
          <div class="grid grid-cols-[120px_1fr] gap-3">
            <span class="text-sm font-medium text-slate-600">Trạng thái:</span>
            <span
              class="inline-flex items-center px-2 py-0.5 rounded-full border text-xs"
              :class="badgeClass(selectedDeposit.trangthai)"
            >
              {{ selectedDeposit.trangthai }}
            </span>
          </div>
    ...
    >
      <div class="bg-white rounded-lg shadow-lg max-w-lg w-full mx-4">
        <div class="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
          <h2 class="text-lg font-semibold text-slate-800">Huỷ phiếu tiền cọc</h2>
          <button
